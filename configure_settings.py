...
def configure_settings(net: Mininet, sender_cca: CongestionControlAlgo, switch_qm: QueueManagement, receiver_ack: ReceiverFeedback):
    ...
    default_ccas = [CongestionControlAlgo.CUBIC, CongestionControlAlgo.RENO]

    # If the CCA is not loaded by default, we need to explicitly load the Kernel module in.
    if sender_cca not in default_ccas:
        subprocess.call(['sudo', 'modprobe', f'tcp_{sender_cca.value}'])
        # The OS also needs to allow the senders to use this CCA.
        os.system(f'sudo sysctl -w net.ipv4.tcp_allowed_congestion_control="cubic reno {sender_cca.value}"')
    ...
    for host in net.hosts:
        host.cmd(f'sudo sysctl -w net.ipv4.tcp_allowed_congestion_control="cubic reno {sender_cca.value}"')
    ...
    quickack_val = '0' if receiver_ack == ReceiverFeedback.DELAYED_ACK else '1'
