...
/* Increased g to 5 to smooth out jitter */
static unsigned int dctcp_shift_g __read_mostly = 5; 
...
static void dctcp_update_alpha(struct sock *sk, u32 flags)
{
	...
	if (!before(tp->snd_una, ca->next_seq)) {
		...
		/* If no CE marks were seen, use g=4 to drop alpha quickly.
		 * If marks were seen, use default g to stay stable against jitter. */
		if (delivered_ce == 0) {
			current_g = 4; 
		} else {
			current_g = dctcp_shift_g; 
		}
		alpha -= min_not_zero(alpha, alpha >> current_g);
		
		if (delivered_ce) {
			u32 delivered = tp->delivered - ca->old_delivered;
			delivered_ce <<= (10 - current_g);
...
static void dctcp_react_to_loss(struct sock *sk)
{
    ...
    /* If alpha is <1%, only reduce congestion window by 1/8. If alpha is greater,
	try to use standard DCTCP reduction instead of Reno. */
    if (ca->dctcp_alpha < 20) { 
